...
// /learn/module/:moduleId/quest — the end-of-module check.
  ...
  return (
    <div className={`ln-${track.id}`} style={{ paddingTop: 20 }}>
      <header className="ln-focus-bar">
        <Link to={`/learn/module/${mod.id}`} className="ln-icon-btn" aria-label="Pause and leave the quest">
          <X size={17} />
        </Link>
        ...
        {done ? (
          <div className="ln-card success">
            ...
            <div className="ln-row" style={{ marginTop: 20 }}>
              <Link to={`/learn/track/nepali-${track.id}`} className="btn btn-green">
                Back to the trail
              </Link>
